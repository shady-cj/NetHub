function focusEffect(field,inputSpan,onFocus){
    ...
    $(container).css({'backgroundColor':bgColor,'border':`1px solid ${coLor}`})
...
function checkInput(field,inputSpan,onFocus){
    if(onFocus){
        ...
        if ($(field).val().trim().length > 0){
            ...
            $(inputSpan).css({'fontSize':'20px','top':'50%','transform':'translateY(-50%)','left':'12px'})
        }
    }
    
}
function activeInput(inputSpan){
    $(inputSpan).css({'fontSize':'10px','top':'20%','transform':'0','left':'20px'})
}